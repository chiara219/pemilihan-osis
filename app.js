(() => {
  ...
  const IMAGE_BASE =
    "https://raw.githubusercontent.com/chiara219/pemilihan-osis/main/images/";
  ...
  function getPhoto(c, role) {
    ...
    /*
     * Kalau spreadsheet/API sudah mempunyai URL foto,
     * gunakan URL tersebut.
     */
    if (
      field &&
      String(field).trim() &&
      /^https?:\/\//i.test(String(field).trim())
    ) {
      return String(field).trim();
    }

    /*
     * Kalau URL kosong, otomatis gunakan foto
     * yang ada di folder images GitHub.
     */
    return (
      IMAGE_BASE +
      "calon" +
  ...
  function photoHTML(c, role, name) {
    const url = getPhoto(c, role);

    const huruf = role === "ketua" ? "K" : "W";
    const jabatan = role === "ketua" ? "KETUA" : "WAKIL";
        ...
        ${
          locked
            ...
            : `
              ...
              <input
                id="cFotoKetua"
                placeholder="Opsional - otomatis menggunakan foto GitHub"
              >
